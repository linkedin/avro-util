 ...
 */
package com.linkedin.avroutil1.compatibility.avro110.parsing;
...
public class JsonGrammarGenerator extends ValidatingGrammarGenerator {
   ...
   * grammar for the given schema {@code sc}.
   */
  @Override
   ...
   * schema {@code sc}. If there is already an entry for the given schema in the
   * given map {@code seen} then that entry is returned. Otherwise a new symbol
  ...
  public Symbol generate(Schema sc, Map<LitS, Symbol> seen, boolean useFqcns) {
    ...
    case ARRAY:
      return Symbol.seq(Symbol.repeat(Symbol.ARRAY_END, Symbol.ITEM_END, generate(sc.getElementType(), seen, useFqcns)),
          Symbol.ARRAY_START);
    case MAP:
      return Symbol.seq(Symbol.repeat(Symbol.MAP_END, Symbol.ITEM_END, generate(sc.getValueType(), seen, useFqcns),
