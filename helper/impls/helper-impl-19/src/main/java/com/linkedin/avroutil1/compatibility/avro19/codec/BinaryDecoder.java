...
public class BinaryDecoder extends Decoder {
  ...
  BinaryDecoder configure(InputStream in) {
    configureSource(8192, new InputStreamByteSource(in));
    return this;
  }
  ...
  BinaryDecoder configure(byte[] data, int offset, int length) {
    configureSource(8192, new ByteArrayByteSource(data, offset, length));
    return this;
  }
   ...
   * InputStream returned by BinaryDecoder.inputStream to access the
