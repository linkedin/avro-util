...
public class BinaryDecoder extends Decoder {
  private static final long MAX_ARRAY_SIZE = 2147483639L;
  ...
  BinaryDecoder configure(InputStream in) {
    this.configureSource(8192, new BinaryDecoder.InputStreamByteSource(in));
    return this;
  }
  ...
  private void configureSource(int bufferSize, BinaryDecoder.ByteSource source) {
    if (null != this.source) {
      this.source.detach();
    }
  ...
  public Utf8 readString(Utf8 old) throws IOException {
    ...
    if (length > 2147483639L) {
      ...
      if (0L != length) {
        this.doReadBytes(result.getBytes(), 0, (int)length);
      }
