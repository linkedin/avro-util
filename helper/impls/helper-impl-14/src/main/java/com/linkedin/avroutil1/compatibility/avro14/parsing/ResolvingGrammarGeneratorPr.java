...
public class ResolvingGrammarGeneratorPr extends ValidatingGrammarGenerator {
      ...
      Map<LitS, Symbol> seen) throws IOException
  {
    ...
    if (writerType == readerType) {
      switch (writerType) {
        ...
        case ARRAY:
          return Symbol
              .seq(Symbol.repeat(Symbol.ARRAY_END, generate(writer.getElementType(), reader.getElementType(), seen)),
                  Symbol.ARRAY_START);

        case MAP:
          return Symbol.seq(Symbol
                  .repeat(Symbol.MAP_END, generate(writer.getValueType(), reader.getValueType(), seen), Symbol.STRING),
              Symbol.MAP_START);
        case RECORD:
      ...
      switch (readerType) {
        case LONG:
          switch (writerType) {
            ...
            case DOUBLE:
            case FLOAT:
              return Symbol.resolve(super.generate(writer, seen, true), Symbol.LONG);
          }
          break;
        ...
        case DOUBLE:
          switch (writerType) {
            ...
            case LONG:
            case FLOAT:
              return Symbol.resolve(super.generate(writer, seen, true), Symbol.DOUBLE);
          }
          break;
  ...
  protected Symbol resolveUnion(Schema writer, Schema reader,
      Map<LitS, Symbol> seen) throws IOException {
    ...
    return Symbol.seq(Symbol.alt(symbols, labels, null, true), new Symbol.WriterUnionAction());
  }
