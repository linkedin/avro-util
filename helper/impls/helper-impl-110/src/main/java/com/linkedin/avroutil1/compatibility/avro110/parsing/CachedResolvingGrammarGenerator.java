  ...
  protected Symbol resolveRecords(Schema writer, Schema reader,
    ...
    if (result == null) {
      ...
      for (Schema.Field wf : wfields) {
        ...
        if (rf == null) {
          production[--count] =
              new CachedSymbol.CachedSkipAction(generate(wf.schema(), wf.schema(), true));
        } else {
          production[--count] =
              generate(wf.schema(), rf.schema(), true);
        }
      }
      ...
      for (Schema.Field rf : rfields) {
        ...
        if (wf == null) {
          ...
          production[--count] = generate(rf.schema(), rf.schema(), true);
