...
public class ResolvingGrammarGenerator extends ValidatingGrammarGenerator {
  ...
  ) throws IOException
  {
    ...
    if (writerType == readerType) {
      ...
      case UNION:
        int j = bestBranch(reader, writer, seen, useFqcns);
        if (j >= 0) {
  ...
  private Symbol resolveUnion(
    ...
    String[] oldLabels = new String[size];
  ...
  private Symbol resolveRecords(
    ...
    if (result == null) {
      ...
      for (Field wf : wfields) {
        ...
        if (rf == null) {
          production[--count] =
            Symbol.skipAction(generate(wf.schema(), wf.schema(), seen, useFqcns));
        } else {
          production[--count] =
  ...
  /**
   * Returns the Avro binary encoded version of {@code n} according to
   * the schema {@code s}.
  ...
  private int bestBranch(Schema r, Schema w, Map<LitS, Symbol> seen, boolean useFqcns) throws IOException {
        ...
        if (vt == b.getType())
          if (vt == Schema.Type.RECORD || vt == Schema.Type.ENUM ||
            ...
            if (vt == Schema.Type.RECORD &&
                !hasMatchError(resolveRecords(w, b, seen, useFqcns))) {
