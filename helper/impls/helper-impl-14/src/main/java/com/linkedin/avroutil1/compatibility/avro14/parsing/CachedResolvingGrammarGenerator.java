...
/**
 * A version of ResolvingGrammarGenerator with a resolveRecords that overrides the method
 * in the base class to use a fixed version of the SkipAction
 * called CachedSkipAction  -- this derives from a copy of ResolvingGrammarGeneratorPr that is fixed
 * to have some protected methods instead of private ones copied from Avro 1.4.0 ResolvingGrammarGenerator
 */
 ...
 * schemas.
 */
public class CachedResolvingGrammarGenerator extends ResolvingGrammarGeneratorPr {
  /**
  ...
  protected Symbol resolveRecords(Schema writer, Schema reader,
    ...
    if (result == null) {
      ...
      for (Schema.Field wf : wfields) {
        ...
        if (rf == null) {
          production[--count] =
              new CachedSymbol.CachedSkipAction(generate(wf.schema(), wf.schema(), seen));
        } else {
          production[--count] =
