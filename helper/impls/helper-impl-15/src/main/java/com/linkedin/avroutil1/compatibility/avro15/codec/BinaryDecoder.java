...
public class BinaryDecoder extends Decoder {
  ...
  BinaryDecoder configure(InputStream in) {
    configureSource(DEFAULT_BUFFER_SIZE, new InputStreamByteSource(in));
    return this;
  }
  ...
  BinaryDecoder configure(byte[] data, int offset, int length) {
    configureSource(DEFAULT_BUFFER_SIZE, new ByteArrayByteSource(
        data, offset, length));
    return this;
  }
   ...
   * the InputStream returned by BinaryDecoder.inputStream to access the
